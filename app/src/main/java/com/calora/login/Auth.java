...
public class Auth extends AsyncTask<String, Void, String> {
    ...
    protected void onPreExecute() {
        ...
        if (getDialog() != null) {
            getDialog().setMessage("Encontrado Usuario...");
            getDialog().show();
        }
    }
	...
	protected void onPostExecute(String s) {
        ...
        if(s == null || s.isEmpty()){
            Toast.makeText(activity,(Html.fromHtml("404: Errors")), Toast.LENGTH_LONG).show();
            return;
        }
        ...
        try {
            ...
            if(data.get("Status").toString().equals("Success")) {
			    ...
			    //getActivity().startService(new Intent(activity, NetworkConnection.class));
				try {
					Toast.makeText(activity,Html.fromHtml("<font color='#FF00FF'>Bem-Vindo! " + data.get("Dias").toString() + "</font>"), Toast.LENGTH_LONG).show();
